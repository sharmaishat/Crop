...
def process_images(uploaded_files, output_directory):
    ...
    for uploaded_file in uploaded_files:
        # Ensure the file pointer is at the start when we open the image
        uploaded_file.seek(0)

        # Generate a unique key for each file
        file_hash = hashlib.md5(uploaded_file.getvalue()).hexdigest()  # Use getvalue() instead of read()
        
        uploaded_file.seek(0)  # Reset file pointer after generating hash
        ...
        try:
            # Open the image directly without reading the file first
